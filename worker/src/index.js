...
/** @param {string} lang @param {ExecutionContext} ctx */
async function getHomeData(lang, ctx) {
  const cacheKey = new Request(`https://internal-cache.vadai/home/${lang}`);
  const cache = caches.default;
  try {
    const cached = await cache.match(cacheKey);
    if (cached) return cached.json();
  } catch { /* ignore cache miss errors */ }
  ...
  const toCache = Response.json(data, { headers: { 'Cache-Control': 'public, max-age=300' } });
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === '/api/home' || path === '/home') {
        ...
        const data = await getHomeData(lang, ctx);
